...
const WarnModal = ({id , show , onHide , isWarned , onReload}) => {
	...
	const { sendRequest:sendRequestForWarnUser, status:statusForWarnUser, data:dataForWarnUser , error:errorForWarnUser } = useHttp(Warn_Or_Block_User);
	const { sendRequest:sendRequestForRemoveWarning, status:statusForRemoveWarning, data:dataForRemoveWarning , error:errorForRemoveWarning } = useHttp(Remove_Warning_Or_Blocking_Badge);
	...
	const WarnUserHandler = () => {
		if(!isWarned){
			setLoading(true)
			const SelectedMessage_ = {"warningMessage" : SelectedWarnMessage ? SelectedWarnMessage : data[0] }
			const Type = 'Warn'
	...
	? <select className='WarnReasons' onChange={(e)=> setSelectedWarnMessage(e.target.value)}>
		{ViewAllReasons}
		</select>
	: <h6 className='fw-bold text-center'> Are You Sure from Removing Warning Badge from this user</h6>
