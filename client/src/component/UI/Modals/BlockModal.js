...
const BlockModal = ({id , show , onHide , isBlocked , onReload}) => {
	...
	const { sendRequest:sendRequestForBlockUser, status:statusForBlockUser, data:dataForBlockUser , error:errorForBlockUser } = useHttp(Warn_Or_Block_User);
	const { sendRequest:sendRequestForRemoveBlocking, status:statusForRemoveBlocking, data:dataForRemoveBlocking , error:errorForRemoveBlocking } = useHttp(Remove_Warning_Or_Blocking_Badge);
	...
	const BlockUserHandler = () => {
		if(!isBlocked){
			setLoading(true)
			const SelectedMessage_ = {"blockReason" : SelectedBlockMessage ? SelectedBlockMessage : data[0] }
			const Type = 'Block'
	...
	: <h6 className='fw-bold'> Are You Sure from Removing Blocking Badge from this user</h6>
