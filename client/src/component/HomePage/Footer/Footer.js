...
import { faEnvelope, faLocationDot, faPhone , faTwiter } from "@fortawesome/free-solid-svg-icons";
...
function Footer() {
	...
	const FooterMoreDetailsData = [
		...
		{text:'Contact Us'  , links:[
			{ name : 'Chat now' , path: '/chat'	, className:'SupportLink' }
		]},
	]
		...
		<Col lg={3} sm={4} xs={12} key={index} >
			...
			<ul>
				{data.links.map((_link, index)  => (
					<li key={index}>
						<Link to={_link.path} className={`text-decoration-none ${classes.footerLinks} ${_link.className && classes[_link.className]}`}>
							{_link.name}
						</Link>
					</li>
					))
